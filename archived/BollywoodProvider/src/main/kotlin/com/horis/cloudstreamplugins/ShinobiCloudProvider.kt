...
class ShinobiCloudProvider : MainAPI() {
    ...
    override var mainUrl = "https://home.shinobicloud.cf"
    override var name = "ShinobiCloud"
    ...
    private suspend fun getDownloadLink(path: String): String {
        val json = tryParseJson<GDConfig>(
            app.get("$path?a=view", headers = headers).text
                .substring("window.UI = JSON.parse('", "');")
        ) ?: throw ErrorLoadingException("load gd config fail")
        return if (json.second_domain_for_dl) {
            path.replace(mainUrl, json.videodomain)
        } else {
            path
        }
    }
    ...
    private fun resDecode(str: String): String {
        val t = str.reversed().substring(24)
        val t1 = t.substring(0, t.length - 20)
        return String(base64Decode(t1).toByteArray(Charsets.ISO_8859_1))
    }
    ...
    data class GDConfig(
        val second_domain_for_dl: Boolean,
        val downloaddomain: String,
        val videodomain: String
    )
